...
class DashboardController extends Controller
{
    ...
    public function getSRKpiTargets(Request $request)
    {
        ...
        if ($request->bar_type=='Monthly') {
            $data  = UserKpiTargetSale::getUserMonthlyKpiTarget(get_user()->id);
        }
        if ($request->bar_type=='Weekly') {
            $data  = UserKpiTargetSale::getUserWeeklyKpiTarget(get_user()->id);
        }
        ...
        foreach ($data as $key => $value) {
           ...
           $chart_value[]=$value->target_value;
        }
        return response()->json(['title'=>$chart_title,'value'=>$chart_value]);
    }
