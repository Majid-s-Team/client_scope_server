...
class Appointment extends Model
{
   ...
   */
    ...
    */
    ...
    public static function getAppointmentDays($params)
    {
        $data = [];
        $query = self::selectRaw("DISTINCT DATE(start_datetime) AS start_datetime")
                    ->where('assignee_user_id',$params['user']->id)
                    ->get();
        if( count($query) ){
            foreach( $query as $record ){
                $start_date = date('Y-m-d',strtotime($record->start_datetime));
                $data[] = $start_date;
            }
        }
        return $data;
