...
$(document).ready(function(){
    ...
    $('.money').each( function(){
        var value = parseFloat($(this).val());
            value = value.toLocaleString('us-US', { style: 'currency', currency: 'USD' })
            value = value.replace('$','')
        $(this).val(value)
    })
})
...
function calculateAllFieldsValues()
{
    ...
    var left_to_sell_value = Math.round( parseFloat(company_annual_sales_target.val()) - parseFloat(company_year_to_date_sold.val()));
    var us_format_left_to_sell_value = left_to_sell_value.toLocaleString('us-US', { style: 'currency', currency: 'USD' })  
    $('#left_to_sell').val(us_format_left_to_sell_value.replace('$',''));

    //company sale needed per week calculate
    var company_sales_needed_per_week_val =  Math.round(parseFloat(left_to_sell_value) / parseFloat(work_week_left_for_the_year.val()));
  ...
  $('#company_sales_needed_per_week').val(company_sales_needed_per_week_val);
    ...
    var company_sales_needed_per_month_val =  Math.round(parseFloat(left_to_sell_value) / parseFloat(work_month_left_for_the_year.val()));
    ...
    var total_sales_reps_needed_val = Math.round( parseFloat(company_annual_sales_target.val()) / parseFloat(average_annual_sales_per_sales_reps.val()) )
